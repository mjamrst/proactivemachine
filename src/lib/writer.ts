...
// Lazy-initialize the Writer client to avoid build-time errors when API key isn't set
let writerClient: Writer | null = null;

function getWriterClient(): Writer {
  if (!writerClient) {
    if (!process.env.WRITER_API_KEY) {
      throw new Error('Writer API key not configured');
    }
    writerClient = new Writer({
...
// Palmyra Creative is optimized for creative thinking, so we adjust the system prompt slightly
...
export async function generateIdeasWithWriter(
  ...
  if (!responseText) {
    throw new Error('No content in Writer response');
  }
  ...
  try {
    ...
    console.error('Failed to parse Writer response:', responseText);
    throw new Error('Failed to parse ideas from Writer response');
  }
}
